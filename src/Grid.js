...
export default class Grid {
     ...
     */
    refresh(rect) {
        this.forEachPoint(rect, (x, y) => {
            const present = this.dataSource(x, y);
            if (present) {
                this.values.set(key(x, y), true);
            } else {
                this.values.delete(key(x, y));
            }
        });
    }
    ...
    isSet(x, y) {
        if (Phaser.Geom.Rectangle.Contains(this.rect, x, y)) {
            return this.values.has(key(x, y));
        }
        switch (this.overflow) {
            case Grid.Overflow.TORUS: {
                let p = this.rectangleMod(x, y);
                return this.values.has(key(p.x, p.y));
            }
